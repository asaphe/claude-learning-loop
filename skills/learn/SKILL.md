...
**Compression must come from the section the addition joins.** If the addition genuinely belongs there, that section is where its redundancy lives; having to compress *unrelated* sections to fund it is positive evidence it belongs elsewhere. So the trip-wire is **the first edit outside the target section**, not exhaustion — "I cannot find enough compression" is unreachable in practice, because there is always one more adjective to shave, and a file that funds a new rule by deleting trigger phrases, named anti-patterns, or scope-clarifying sentences from elsewhere has gotten worse while staying the same size.

When the bound trips, the excess does NOT go into the anti-bloat-target file. Re-route: prefer another always-loaded tier if the manifest offers one, else a `doc:<topic>`. If the target section has itself grown past the file's density (a bullet carrying an enumerated progression, say), the right move is usually to extract *that whole section* to its own file behind a one-line pointer — which pays for the addition and shrinks the file, instead of taxing six unrelated sections to break even. Say plainly in the closing report which candidates re-routed and why.
